...
interface ViewOnExplorerProps extends HTMLAttributes<SVGElement> {
  symbol: string
  text?: string
}

export const ViewIcon: React.FC<React.SVGProps<SVGSVGElement>> = () => {
  return (
    <svg fill="none" height="15" viewBox="0 0 9 9" width="16" xmlns="http://www.w3.org/2000/svg">
      <path
        clipRule="evenodd"
...
export const ViewOnExplorer: React.FC<ViewOnExplorerProps> = ({ className, symbol, text }) => {
  ...
  return (
    <Container>
      <Tooltip content={`View ${symbol} on explorer`}>
        <Container>
          <SVGButton className={className} onClick={viewOnExplorer}>
            <ViewIcon />
          </SVGButton>
          {text ? <TextButton onClick={viewOnExplorer}>{text}</TextButton> : ''}
        </Container>
      </Tooltip>
    </Container>
  )
