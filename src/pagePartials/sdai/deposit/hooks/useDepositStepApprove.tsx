...
export const useDepositStepApprove = ({
  ...
  const savingsXDaiAdapter = useContractInstance(SavingsXDaiAdapter__factory, 'SavingsXDaiAdapter')
  const erc20 = useContractInstance(ERC20__factory, tokenAddress, true)
  const sendTx = useTransaction()
  const { refetchAllowance } = useGetERC20Allowance(tokenAddress, savingsXDaiAdapter.address)

  const approve = useCallback(async () => {
    const approvalAmount = unlimitedApprovalRef.current
      ? '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF'
      : amount
    const tx = await sendTx(() => erc20.approve(savingsXDaiAdapter.address, approvalAmount))
    const receipt = await tx.wait()
    await refetchAllowance()
    return receipt.transactionHash
  }, [sendTx, refetchAllowance, erc20, savingsXDaiAdapter.address, amount])
